...
import static ru.bcs.perseus.quotes.model.InstrumentType.BOND;
...
import ru.bcs.perseus.quotes.model.dto.QuoteManualUpdate;
import ru.bcs.perseus.quotes.model.exception.BadRequestException;
...
public class QuotesService {
  ...
  public List<Quote> getQuotesByTypesOnDate(List<InstrumentType> types, LocalDate date) {
    return repository.findAllByTypeInAndDate(types, date);
  }
  ...
  //FIXME REMOVE after csv upload
  public void insert(List<Quote> quotes) {
    repository.saveAll(quotes);
  }

  public Optional<Quote> findLastUpdatedDateQuote(String source) {
    return repository.findFirstBySourceOrderByDateDesc(source);
  }
