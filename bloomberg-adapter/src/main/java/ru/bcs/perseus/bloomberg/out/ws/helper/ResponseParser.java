...
public class ResponseParser {
  ...
  public static List<InstrumentV2> retrieveInstrumentsV2(List<InstrumentData> instrumentDataList,
      List<String> fields) {
    return instrumentDataList.stream()
        .map(instrumentData -> createInstrumentV2(instrumentData, fields))
        .collect(Collectors.toList());
  }
  ...
  private static Dividend getDividend(String figi, BulkArray bulkArray) {
    ...
    final BulkArrayEntry unparsedDeclaredDate = getFieldFromBulkArray(
        bulkArrayEntries,
        DECLARED_DATE_ORDER
    );
    final BulkArrayEntry unparsedExDividendDate = getFieldFromBulkArray(
        bulkArrayEntries,
        EX_DIVIDEND_DATE_ORDER
    );
    final BulkArrayEntry unparsedRecordDate = getFieldFromBulkArray(
        bulkArrayEntries,
        RECORD_DATE_ORDER
    );
    final BulkArrayEntry unparsedPayDate = getFieldFromBulkArray(
        bulkArrayEntries,
        PAY_DATE_ORDER
    );
    final BulkArrayEntry unparsedAmount = getFieldFromBulkArray(
        bulkArrayEntries,
        AMOUNT_ORDER
    );
    final BulkArrayEntry unparsedFrequency = getFieldFromBulkArray(
        bulkArrayEntries,
        FREQUENCY_ORDER
    );
    final BulkArrayEntry unparsedDividendType = getFieldFromBulkArray(
        bulkArrayEntries,
        DIVIDEND_TYPE_ORDER
    );

    final LocalDate declaredDate = getDateField(
        unparsedDeclaredDate != null ? unparsedDeclaredDate.getValue() : null
    );
    final LocalDate exDividendDate = getDateField(
        unparsedExDividendDate != null ? unparsedExDividendDate.getValue() : null
    );
    final LocalDate recordDate = getDateField(
        unparsedRecordDate != null ? unparsedRecordDate.getValue() : null
    );
    final LocalDate payDate = getDateField(
        unparsedPayDate != null ? unparsedPayDate.getValue() : null
    );
    final BigDecimal amount = getBigDecimalField(
        unparsedAmount != null ? unparsedAmount.getValue() : null
    );

    final String frequency = unparsedFrequency != null ? unparsedFrequency.getValue() : null;
    final String dividendType = unparsedDividendType != null
        ? unparsedDividendType.getValue()
        : null;
  ...
  private static InstrumentV2 createInstrumentV2(InstrumentData instrumentData,
      List<String> fields) {
    Map<String, String> fieldValues = makeFieldValues(instrumentData, fields);
    log.debug("Field values for instrument from bloomberg:{}", fieldValues.toString());
    return InstrumentV2Builder.buildInstrumentV2(fieldValues);
  }
  ...
  private static Instrument createInstrument(Map<String, String> fieldValues) {
    ...
    switch (marketSector) {
      case EQUITY:
        return EquityHelper.createEquity(fieldValues);
      case CORP:
      case GOVT:
        return BondHelper.createBond(fieldValues);
      default:
        return null;
    }
  }
